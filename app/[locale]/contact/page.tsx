...
export default function ContactPage() {
  // Օգտագործում ենք 'contact' բաժինը messages JSON-ից
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] text-black p-8 font-sans uppercase">
      <header className="mb-20 border-b border-black pb-4 flex justify-between items-end">
        {/* Թարգմանված վերնագիր */}
        <h1 className="text-3xl font-light italic tracking-tighter">{t('header')}</h1>
      ...
      <div className="max-w-2xl mx-auto border border-black p-10 bg-white shadow-[10px_10px_0px_0px_rgba(0,0,0,0.05)]">
        {submitted ? (
          <div className="py-20 text-center space-y-4 animate-in fade-in zoom-in duration-500">
            <h2 className="text-4xl font-black italic">{t('success')}</h2>
            <p className="text-[10px] opacity-60 tracking-widest">{t('successDesc')}</p>
            <button 
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            <div className="flex flex-col gap-2">
              <label className="text-[10px] tracking-[0.3em] font-mono opacity-40">{t('name')}</label>
              <input 
                required 
                type="text" 
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] tracking-[0.3em] font-mono opacity-40">{t('email')}</label>
              <input 
                required 
                type="email" 
                className="bg-transparent border-b border-black/20 py-2 outline-none focus:border-black transition-colors font-bold italic"
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] tracking-[0.3em] font-mono opacity-40">{t('message')}</label>
              <textarea 
                required 
                rows={3}
